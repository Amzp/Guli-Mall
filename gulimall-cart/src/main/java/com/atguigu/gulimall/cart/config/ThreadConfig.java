...
public class ThreadConfig {
     ...
     *             - TimeUnit.SECONDS：指定存活时间单位为秒。
     *             - new LinkedBlockingDeque<>(100000)：创建一个容量为 100,000 的无界阻塞队列，用于存储待处理的任务。
     *             - Executors.defaultThreadFactory()：使用默认的线程工厂，创建新线程时使用默认命名规则和优先级。
     *             - ThreadPoolExecutor.AbortPolicy()：当线程池和队列已满，新提交的任务将抛出 RejectedExecutionException。
     *
